...
import { timeToMinutes, minutesToTime, durationMinutes } from '@renderer/lib/timeUtils'
import { SNAP_MINUTES } from '@renderer/lib/constants'
...
export function useDragMove({ weekColumnsRef, weekDates, onSave }: UseDragMoveOptions): {
  ...
  const handleDragMoveStart = useCallback(
    (e: React.MouseEvent, entry: LogEntry) => {
      ...
      const col = weekColumnsRef.current?.find((_, i) => weekDates[i] === entry.date)
      const colEl = weekColumnsRef.current?.[weekDates.indexOf(entry.date)]
